...
def _surgical_rebuild_config(smbios_data, mac_addr, custom_config_path, output_path, resolution=None, igpu_patch_properties=None):
    """
    Takes a known-good KVM config.plist and injects per-VM data, including screen resolution.
    """
    ...
    try:
        ...
        # 2. Inject iGPU properties if present
        ...
        # 3. Inject Screen Resolution for HiDPI
        nvram_section = config_data.setdefault("NVRAM", {}).setdefault("Add", {})
        opencore_vendor_guid = "4D1EDE05-38C7-4A6A-9CC6-4BCCA8B38C14"
        
        if resolution:
            print_info(f"  - Injecting resolution for HiDPI: {resolution}")
            # Ensure the vendor GUID dictionary exists
            vendor_nvram = nvram_section.setdefault(opencore_vendor_guid, {})
            # Set UIScale to 02 for HiDPI
            vendor_nvram["UIScale"] = b'\x02'
            # Format and encode the resolution string
            resolution_str = f"{resolution}@2"
            vendor_nvram["resolution"] = resolution_str.encode('ascii')
        else:
            # If no resolution is passed, ensure the keys are removed
            if opencore_vendor_guid in nvram_section:
                nvram_section[opencore_vendor_guid].pop("UIScale", None)
                nvram_section[opencore_vendor_guid].pop("resolution", None)
                print_info("  - Ensuring no custom resolution is set.")
...
def _build_and_patch_opencore_image(vm_name, smbios_data, igpu_patch_properties=None):
    ...
    # Read resolution from the VM's config file
    resolution = None
    if os.path.exists(paths['config']):
        try:
            ...
            print_warning("Could not read resolution from config file.")
    ...
    try:
        ...
        if not os.path.exists(temp_config_path):
             print_error(f"FATAL: Template 'config.plist' not found in 'assets/EFI/OC/'")
             return False

        if not _surgical_rebuild_config(
            smbios_data, mac_addr, temp_config_path, temp_config_path, resolution, igpu_patch_properties
        ):
...
def create_new_macos_vm():
    ...
    if native_res and questionary.confirm(f"Use native host resolution ({native_res})? (Recommended for HiDPI/Retina)").ask():
...
def nuke_and_recreate_macos_vm():
    ...
    if nuke_choice == "identity":
        ...
        if _build_and_patch_opencore_image(vm_name, smbios_data):
...
"""
    ...
    if selected_gpu['type'] == 'iGPU':
        ...
        igpu_patch_properties = {f"PciRoot(0x0)/Pci(0x2,0x0)": KVM_IGPU_PATCHES[profile_choice]}
