...
"""
This module handles all Linux VM management functionality.
"""
import os
import subprocess
import sys
...
def check_dependencies():
    ...
    if not info:
        print_warning(f"Unsupported host distro ('{distro_id}'). "
                      "Please manually install 'qemu' and 'ovmf/edk2' packages.")
    ...
    uefi_path = "/usr/share/OVMF/OVMF_CODE.fd" \
        if distro_id in ["debian", "ubuntu"] else CONFIG['UEFI_CODE']
    ...
    print_info(f"To fix this, the recommended command is:\n  "
               f"{Style.BOLD}{install_cmd}{Style.ENDC}")
...
def get_running_vm_info(vm_name):
    """
    Returns a dictionary of information about a running VM.
    """
...
def cleanup_stale_sessions():
    ...
    if not os.path.isdir(vms_dir):
        return
    vm_list = [d for d in os.listdir(vms_dir)
               if os.path.isdir(os.path.join(vms_dir, d))]
    for vm in vm_list:
        is_vm_running(vm)
...
def create_new_vm():
    ...
    while True:
        vm_name = input(f"{Style.BOLD}Enter a short name for new VM "
                        f"(e.g., arch-kde): {Style.ENDC}").strip()
    ...
    while True:
        disk = input(f"{Style.BOLD}Enter base disk size (GB) "
                     f"[default: {CONFIG['BASE_DISK_SIZE']}]: {Style.ENDC}").strip() \
               or CONFIG['BASE_DISK_SIZE']
        if disk.isdigit() and int(disk) > 0:
            break
        print_warning("Invalid input.")

    vm_settings = get_vm_config({"VM_MEM": CONFIG['VM_MEM'], "VM_CPU": CONFIG['VM_CPU']})
    qemu_cmd = _get_qemu_command(vm_name, vm_settings, {},
                                 {'uuid': str(uuid.uuid4()), 'mac': ''}, "", 0,
                                 iso_path=iso_path)
    ...
    commands_to_run = [
        ...
        (f"Creating {disk}G base image",
         ["qemu-img", "create", "-f", "qcow2", paths['base'], f"{disk}G"]),
...
def _get_qemu_command(vm_name, vm_settings, input_devices, ids, host_dns,
    ...
    if ssh_port > 0:
        qemu_cmd.extend(["-netdev",
                         f"user,id=n1,dns={host_dns},hostfwd=tcp::{ssh_port}-:22",
                         "-device", f"virtio-net-pci,netdev=n1,mac={ids['mac']}"])
    else:
        qemu_cmd.extend(["-netdev", "user,id=n1", "-device", "virtio-net-pci,netdev=n1"])

    if passthrough_info:
        is_gpu_passthrough = any(d.get('class_code') == '0300'
                                 for d in passthrough_info['devices'].values())
        if is_gpu_passthrough:
            qemu_cmd.extend(["-cpu", "host,kvm=off,hv_vendor_id=null"
            if passthrough_info['vendor'] == "NVIDIA" else "host"])
            qemu_cmd.append("-nographic")
            if input_devices:
                qemu_cmd.extend([
                    "-object", f"input-linux,id=mouse,evdev={input_devices['mouse']}",
                    "-object",
                    f"input-linux,id=kbd,evdev={input_devices['keyboard']},grab_all=on,repeat=on"
                ])
        ...
        qemu_cmd.extend(["-cpu", "host", "-vga", "virtio", *CONFIG["QEMU_DISPLAY"]])
        qemu_cmd.extend(["-drive",
                         f"file={paths['overlay']},if=virtio,cache=writeback"])
...
def run_or_nuke_vm(vm_name, is_fresh):
    ...
    if not os.path.exists(paths['overlay']):
        run_command_live(["qemu-img", "create", "-f", "qcow2", "-b",
                         paths['base'], "-F", "qcow2", paths['overlay']],
                        as_root=False, check=True)
...
def _nuke_session_files(paths):
    files_to_nuke = [paths['overlay'], paths['instance'],
                     paths['session_id'], paths['pid_file'], paths['session_info']]
    if not any(os.path.exists(f) for f in files_to_nuke):
        print_info("No session files found to nuke for this VM.")
        return True

    print_warning(f"You are about to permanently delete the current session for "
                  f"'{os.path.basename(paths['dir'])}'.")
...
def transfer_files():
    """
    Transfers files to/from a running VM.
    """
    clear_screen()
    ...
    while True:
        direction = input(f"{Style.BOLD}Direction? "
                        f"[1] Host to VM, [2] VM to Host: {Style.ENDC}").strip()
        if direction in ["1", "2"]:
            break
        print_warning("Invalid choice.")

    while True:
        local_path = os.path.expanduser(input(f"{Style.BOLD}Enter path on "
                                              f"LOCAL host: {Style.ENDC}").strip())
        if os.path.exists(local_path):
            break
        print_warning(f"Local path not found: '{local_path}'. Please check typos.")

    remote_path = input(f"{Style.BOLD}Enter full path on "
                        f"REMOTE vm (use quotes if needed): {Style.ENDC}").strip()
    vm_user = input(f"{Style.BOLD}Enter username inside the VM: {Style.ENDC}").strip()

    port = vm_info['port']
    src, dest = (local_path, f"{vm_user}@localhost:{remote_path}") \
        if direction == "1" else (f"{vm_user}@localhost:{remote_path}", local_path)
...
def _fix_grub_cmdline(param_to_add):
    ...
    if param_to_add in original_content:
        print_info(f"Parameter '{param_to_add}' already exists in GRUB config. "
                   f"No changes needed.")
        return

    new_content = re.sub(r'^(GRUB_CMDLINE_LINUX_DEFAULT=")(.*)(")',
                         rf'\1\2 {param_to_add}\3', original_content, 1, re.M)
    if new_content == original_content:
        print_error("Could not automatically modify GRUB_CMDLINE_LINUX_DEFAULT. "
    ...
    if run_command_live(['cp', tmp_path, grub_file], as_root=True) is not None:
        ...
        update_cmd = DISTRO_INFO.get(distro, {}).get(
            "grub_update", "sudo update-grub (or equivalent for your OS)")
        print_warning("ACTION REQUIRED: You must now apply this change and reboot.")
        print_info(f"Run the command: {Style.BOLD}{update_cmd}{Style.ENDC} and then "
                   f"{Style.BOLD}sudo reboot{Style.ENDC}")
    else:
        os.remove(tmp_path)
...
def _check_iommu_support():
    ...
    print_info("This is usually because it's disabled in the BIOS/UEFI or "
               "the required kernel parameter is missing.")

    param = "intel_iommu=on" if vendor == "Intel" else "amd_iommu=on"
    if input(f"Would you like this tool to attempt to add the '{param}' "
             f"kernel parameter to GRUB? (y/N): ").lower() == 'y':
...
def _select_guest_gpu():
    ...
    if not gpus:
        print_error("FATAL: No 'VGA compatible controllers' with PCI Class [0300] "
                    "and an active kernel driver were found.")
        return None
    ...
    if len(gpus) == 1:
        ...
        vendor = "NVIDIA" if gpu['ids'].startswith("10de") \
            else "AMD" if gpu['ids'].startswith("1002") else "Intel"
        if vendor in ["NVIDIA", "AMD"] and not is_apu:
            print_error(f"FATAL: Single {vendor} dGPU detected. "
                        f"Passthrough requires a second GPU.")
            return None
        if is_apu:
            print_warning("Single AMD APU detected. "
                          "This implies a single-GPU passthrough (headless host).")
            if input("Acknowledge and proceed? (y/N): ").strip().lower() != 'y':
                return None
            return gpu
    elif len(gpus) > 1:
        print_info("Multiple GPUs detected. "
                   "This is a supported 'Two-GPU Passthrough' configuration.")
        return select_from_list(gpus, "Select the GPU to pass through to the GUEST VM",
                                display_key='display')
...
def _get_full_iommu_group_devices(pci_id, groups_out):
    device_group_num = next((line.split()[2] for line in groups_out.splitlines()
                             if pci_id in line), None)
    ...
    for line in groups_out.splitlines():
        if f"IOMMU Group {device_group_num}" in line:
            parts = line.split()
            if len(parts) > 3 and re.match(r'^[\da-f:.]+', parts[3]):
...
def _check_vfio_binding_status(guest_gpu):
    ...
    if guest_gpu['driver'] == 'vfio-pci':
        ...
        print_success(f"GPU is currently bound to its host driver ('{guest_gpu['driver']}'). "
                      f"This is correct for Live Passthrough.")
    return True
...
def _check_nvidia_quirks(guest_gpu):
    ...
    print_info(f"This tool will automatically apply {Style.BOLD}"
               f"-cpu host,kvm=off,hv_vendor_id=null{Style.ENDC} at launch.")
    return True, "NVIDIA"
...
def _check_and_load_vfio_module():
    ...
    print_warning("This tool can create a configuration file to load it automatically:")
    ...
    if input("Create this file now? (y/N): ").strip().lower() == 'y':
        ...
        if run_command_live(['cp', tmp_path, conf_file], as_root=True) is not None:
            ...
            print_info(f"Please run '{Style.BOLD}sudo reboot{Style.ENDC}' "
                       f"and then run this script again.")
        else:
...
def _check_iommu_groups_sanity(guest_gpu):
    ...
    is_clean = True
    group_members = [line for line in groups_out.splitlines()
                     if f"IOMMU Group {group_num}" in line]
    for member in group_members:
        ...
        if is_vga and guest_gpu['pci'] not in member:
            ...
            is_clean = False
        if any(c in member for c in ["USB", "SATA", "Ethernet", "Non-Volatile memory"]) \
                and not is_vga:
...
def run_gpu_passthrough_check():
    ...
    if is_laptop:
        print_warning("Laptop detected. Passthrough is extremely difficult "
                      "and success is highly unlikely.")
    else:
...
def _detect_display_manager():
    ...
    for dm in ["gdm", "lightdm", "sddm", "lxdm", "xdm"]:
        result = subprocess.run(["systemctl", "is-active", f"{dm}.service"],
...
def find_input_devices():
    ...
    print_warning("The selected devices will become unavailable to the host "
    ...
    for block in content.strip().split('\n\n'):
        ...
        if event_dev:
            devices.append({"name": name, "event": event_dev,
                            "path": f"/dev/input/{event_dev}"})
    ...
    if not keyboards or not mice:
        ...
        print_info("You may need to manually identify the /dev/input/eventX "
                   "paths for your devices.")
        return None
...
def run_vm_with_live_passthrough():
    """
    Runs a VM with live passthrough.
    """
    clear_screen()
    print_header("Run VM with Live Passthrough")

    if not _check_and_load_vfio_module():
        print_error("Launch aborted. Please reboot and try again "
                    "after `vfio-pci` module is loaded.")
        return
    ...
    if dm_service is None and os.environ.get("DISPLAY"):
        ...
        print_warning("A graphical session (X11/Wayland) is running, but it was not started "
                      "by a recognized display manager service (gdm, sddm, etc.).")
        ...
        print_info("To proceed, please do the following:\n  1. Log out of your graphical session."
                   f"\n  2. Switch to a TTY (text console) using {Style.BOLD}"
                   f"Ctrl+Alt+F3{Style.ENDC}.\n  3. Log in there and run this script again.")
        return
    ...
    while True:
        ...
        print(f"Current devices: {list(d['display'] for d in passthrough_devices.values())}"
              " or 'None'")
        choice = input(f"{Style.BOLD}Add device: [1] GPU, [2] USB Controller, "
                       f"[3] NVMe Drive, [4] Done: {Style.ENDC}").strip()
        if choice == '1':
            devices = _get_gpus()
            name = "GPU"
        elif choice == '2':
    ...
    for pci_id in passthrough_devices:
        _, group_pci_ids, group_vendor_ids = _get_full_iommu_group_devices(pci_id,
                                                                          iommu_groups_out)
        ...
        for dev_id in group_pci_ids:
            if not re.match(r'^[\da-f:.]+', dev_id):
                print_error(f"FATAL: Collected an invalid device ID '{dev_id}' "
                            f"for IOMMU group of {pci_id}. Aborting.")
                return
    ...
    if dm_service:
        print_warning("\nCRITICAL WARNING: "
                      "This process will stop your graphical desktop session.")
        if is_laptop:
            print(f"{Style.FAIL}{Style.BOLD}Your built-in screen WILL go black. "
                  f"This is NORMAL.{Style.ENDC}")
            print(f"{Style.OKGREEN}To see the VM, you MUST connect an external monitor "
                  f"to your laptop's HDMI/DisplayPort.{Style.ENDC}")
        else:
            print(f"{Style.FAIL}{Style.BOLD}Your primary monitor WILL go black. "
                  f"This is NORMAL.{Style.ENDC}")
            print(f"{Style.OKGREEN}You must connect a second monitor to the "
                  f"passed-through GPU to see the VM.{Style.ENDC}")
        print(f"{Style.OKGREEN}Your desktop will automatically return "
              f"when the VM shuts down.{Style.ENDC}")
    ...
    ids = {'uuid': str(uuid.uuid4()),
           'mac': f"52:54:00:{random.randint(0, 255):02x}:"
                  f"{random.randint(0, 255):02x}:{random.randint(0, 255):02x}"}
    vendor = "NVIDIA" if any(dev['ids'].startswith('10de')
                             for dev in passthrough_devices.values()) else ""
    primary_gpu = next((d for d in passthrough_devices.values()
                        if d['class_code'] == '0300'), None)
    passthrough_info = {
    ...
    qemu_cmd_list = _get_qemu_command(vm_name, vm_settings, input_devices,
                                      ids, host_dns, ssh_port, passthrough_info)

    with open(script_path, 'w', encoding='utf-8') as f:
        f.write("#!/bin/bash\n")
        f.write("# This script requires root. It will be executed with sudo.\n")
        f.write(f"exec &> {log_path}\n")
        f.write("set -x\n")

        post_cmds = []
        if dm_service:
            post_cmds.append("echo 'Restarting graphical session...' && "
                             f"systemctl start {dm_service}")
        for pci_id in reversed(sorted(list(final_pci_ids_to_bind))):
            original_driver = original_drivers.get(pci_id)
            if original_driver:
                post_cmds.insert(0, f"echo 'Rebinding {pci_id} to {original_driver}' && "
                                    f"echo {pci_id} > "
                                    f"/sys/bus/pci/drivers/{original_driver}/bind || true")
                post_cmds.insert(0, f"echo {pci_id} > "
                                    f"/sys/bus/pci/drivers/vfio-pci/unbind || true")
        for vdid in final_vendor_ids_to_register:
            post_cmds.insert(0, f"echo {vdid.replace(':', ' ')} > "
                                f"/sys/bus/pci/drivers/vfio-pci/remove_id || true")
        ...
        for vdid in final_vendor_ids_to_register:
            f.write(f"echo {vdid.replace(':', ' ')} > "
                    f"/sys/bus/pci/drivers/vfio-pci/new_id || true\n")
        for pci_id in final_pci_ids_to_bind:
            f.write(f"echo 'Unbinding {pci_id}' && echo {pci_id} > "
                    f"/sys/bus/pci/devices/{pci_id}/driver/unbind || true\n")
            f.write(f"echo 'Binding {pci_id} to vfio-pci' && echo {pci_id} > "
                    f"/sys/bus/pci/drivers/vfio-pci/bind\n")

        f.write("\n\necho '--- Launching QEMU ---\n")
        f.write(' '.join(shlex.quote(s) for s in qemu_cmd_list))
        f.write("\n\necho '--- QEMU process finished, exiting script. "
    ...
    print_warning("Your screen should go black shortly. "
                  f"To debug, check the log file at: {log_path}")

    launch_cmd = ['nohup', 'sudo', 'bash', script_path]
    with open(os.devnull, 'w', encoding='utf-8') as devnull:
        subprocess.Popen(launch_cmd, stdout=devnull, stderr=devnull, preexec_fn=os.setsid)
    sys.exit(0)


def revert_system_changes():
    """
    Reverts all system changes made by the script.
    """
    clear_screen()
    ...
    update_initramfs_cmd = DISTRO_INFO.get(distro, {}).get("initramfs_update",
                                                           "sudo update-initramfs -u")
    print_warning("ACTION REQUIRED: To finalize the revert, "
                  "you may need to update system configs and reboot.")
    print_info("If GRUB or initramfs were affected, run these commands:\n"
               f"  {Style.BOLD}{update_grub_cmd}\n  {update_initramfs_cmd}{Style.ENDC}")
    print_info(f"Then reboot with: {Style.BOLD}sudo reboot{Style.ENDC}")


def display_passthrough_guide():
    """
    Displays a guide on what to expect and how to use passthrough.
    """
    clear_screen()
    print_header("What to Expect & How to Use Passthrough")
    print(f"""
{Style.BOLD}Understanding the "Headless Host" Concept{Style.ENDC}
When you pass a GPU to a VM, your main operating system (the "host") can no longer use it.
For this to work without crashing, we must completely stop the host's graphical
desktop environment before the VM starts.

{Style.WARNING}This means your screen WILL go black and show a text cursor.
This is normal!{Style.ENDC}

The host is now "headless" (it has no display). The VM, however,
now has full control of the GPU.
...
You need to connect a monitor to a port that is physically wired to the passed-through GPU.
  ...
  - {Style.OKCYAN}For Laptops:{Style.ENDC}
    This almost always means you {Style.BOLD}MUST connect an external monitor or TV{Style.ENDC}
    to your laptop's {Style.OKGREEN}HDMI or DisplayPort{Style.ENDC}.
  ...
  - {Style.OKCYAN}For Desktops:{Style.ENDC}
    You need two monitors. One connected to your host GPU, and a second one connected
    to the GPU you are passing to the VM.
...
If you do not have an external monitor, you can install remote desktop software
(like VNC, XRDP, or NoMachine) inside your VM's operating system.
You can then connect to the VM's desktop from another computer on your network.
    """)


def gpu_passthrough_menu():
    """
    Displays the GPU passthrough menu.
    """
    ...
    while True:
        ...
        print(f"\n{Style.HEADER}{Style.BOLD}Passthrough & Performance (Advanced){Style.ENDC}\n"
              "───────────────────────────────────────────────")
        print(f"{Style.OKBLUE}1.{Style.ENDC} {Style.BOLD}"
              f"Run VM with 'Live' Passthrough (GPU, USB, NVMe){Style.ENDC}")
        print(f"{Style.OKCYAN}2.{Style.ENDC} {Style.BOLD}"
              f"Run System Compatibility Checklist{Style.ENDC}")
        print(f"{Style.OKGREEN}3.{Style.ENDC} {Style.BOLD}"
              f"What to Expect & How to Use Passthrough{Style.ENDC}")
        print(f"{Style.FAIL}4.{Style.ENDC} {Style.BOLD}"
              f"Revert ALL Passthrough-Related System Changes{Style.ENDC}")
...
def linux_vm_menu():
    ...
    while True:
        ...
        print(f"{Style.OKCYAN}2.{Style.ENDC} {Style.BOLD}"
              f"Run / Resume VM Session (Standard Graphics){Style.ENDC}")
        print(f"{Style.OKBLUE}3.{Style.ENDC} {Style.BOLD}Nuke & Boot a Fresh Session{Style.ENDC}")
        print(f"{Style.OKGREEN}4.{Style.ENDC} {Style.BOLD}"
              f"Transfer Files to/from a Running VM{Style.ENDC}")
        print(f"{Style.OKCYAN}5.{Style.ENDC} {Style.BOLD}"
              f"Passthrough & Performance (Advanced){Style.ENDC}")
